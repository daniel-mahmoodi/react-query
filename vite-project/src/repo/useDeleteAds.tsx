...
import { OldStateType } from "./models";
...
function useDeleteAds() {
  ...
  const { isPending, mutate, mutateAsync } = useMutation({
    ...
    mutationFn: (data:any) => {
      const deletePromise = new Promise((resolve, reject) => {
        const deleteTimeout = setTimeout(() => {
          axios
            .delete(`http://127.0.0.1:8000/ads/advertises/${data.id}/`)
            .then(resolve)
            .catch(reject);
        }, 8000);
    ...
    onMutate: (data) => {
      const oldState: OldStateType = client.getQueryData(["Ads"]);
      if (oldState) {
        const newData = oldState.pages[0].results.filter(
          (e) => e.id != data.id
        );
