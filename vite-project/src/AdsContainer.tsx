...
import  { AdsPageParams } from "./repo/useAdsPaginated.tsx";
...
function AdsContainer() {
  ...
  return (
    <>
      ...
      <span
        onClick={() =>
          queryClient.cancelQueries({ queryKey: ["Ads", pageParams] })
        }
      >
        Cancel
      </span>
      <input
        type="text"
        onChange={(e) =>
          setPageParams({ ...pageParams, search: e.target.value })
        }
      />
      ...
      <div>
        <span onClick={() => setPageParams({ ad_type: "all", page: 1 })}>
          All
        </span>{" "}
        |
        <span onClick={() => setPageParams({ ad_type: "buy", page: 1 })}>
          Buy
        </span>{" "}
        |
        <span onClick={() => setPageParams({ ad_type: "sell", page: 1 })}>
          Sell
        </span>{" "}
        |
        {deletePending && (
          <h2 onClick={() => undoDelete.current && undoDelete.current()}>
            Deleting click to undo
          </h2>
        )}
      </div>
      {data && (
      ...
      {/*</div>*/}
      <div
        className={`fixed w-screen bottom-0 left-0 bg-white transition-all duration-200 ${
          showDrawer ? "h-1/2" : "h-0"
        }`}
      >
        {showDrawer && (
          <AdsForm
            initialData={selectAds}
            onDelete={() => {
            ...
            onSubmit={(data) => {
              if (selectAds) {
                ...
                mutateAsync({ ...data, image: data.image[0] }).then(() =>
                  setShowDrawer(false)
                );
              }
            }}
